...
export default function DemoCV() {
  ...
  // Auto-start processing when section enters view on mobile
  useEffect(() => {
    if (!isMobile || !isInView || demoState !== 'drop' || prefersReducedMotion) return
    const t = setTimeout(() => startProcessing(), 600) // eslint-disable-line react-hooks/exhaustive-deps
    return () => clearTimeout(t)
  }, [isMobile, isInView, demoState, prefersReducedMotion]) // eslint-disable-line react-hooks/exhaustive-deps

  // Reset when section leaves view on mobile
  useEffect(() => {
  ...
  return (
    <section id="demo-cv" className="bg-white" ref={sectionRef}>
